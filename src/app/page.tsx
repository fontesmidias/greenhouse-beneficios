...
export default function Home() {
  ...
  const [resendMenuFor, setResendMenuFor] = useState<string | null>(null);
  ...
  // Fecha o dropdown de Reenviar ao clicar fora ou apertar Esc
  useEffect(() => {
    if (!resendMenuFor) return;
    const onClick = () => setResendMenuFor(null);
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') setResendMenuFor(null); };
    // Pequeno delay para o click do botão de abrir não ser capturado imediatamente
    const t = setTimeout(() => {
  ...
  const resendReceipt = async (rec: any, channel: 'email' | 'whatsapp' | 'both') => {
    setResendMenuFor(null);
    setResending(rec.id);
    setFeedback(null);
    try {
      const res = await fetch(`/api/receipts/${rec.id}/resend`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        const partes: string[] = [];
        if (data.sent?.email) partes.push('e-mail');
        if (data.sent?.whatsapp) partes.push('WhatsApp');
        const erros: string[] = [];
        if (data.errors?.email) erros.push(`e-mail: ${data.errors.email}`);
        if (data.errors?.whatsapp) erros.push(`WhatsApp: ${data.errors.whatsapp}`);
        const txt = `Reenviado com sucesso por ${partes.join(' e ')} para ${rec.nome}.${erros.length ? ' Avisos: ' + erros.join(' | ') : ''}`;
        setFeedback({ type: erros.length ? 'warning' : 'success', text: txt });
      } else {
        const erros: string[] = [];
        if (data.errors?.email) erros.push(`e-mail: ${data.errors.email}`);
        if (data.errors?.whatsapp) erros.push(`WhatsApp: ${data.errors.whatsapp}`);
        setFeedback({
          type: 'error',
          text: `Falha ao reenviar para ${rec.nome}.${erros.length ? ' ' + erros.join(' | ') : ' ' + (data.error || 'Motivo desconhecido.')}`,
        });
      }
    } catch (err: any) {
      setFeedback({ type: 'error', text: `Erro de rede ao reenviar: ${err?.message || err}` });
    } finally {
      setResending(null);
    }
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2">
                { (session.user as any).role === 'ADMIN' && (
                  <>
                    <a href="/admin/orphans" className="w-8 h-8 bg-amber-500/10 hover:bg-amber-500/20 rounded-xl flex items-center justify-center text-amber-400 transition-all ring-1 ring-amber-500/20" title="Arquivos órfãos no volume (Admin)">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                    </a>
                    <a href="/admin/users" className="w-8 h-8 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-xl flex items-center justify-center text-emerald-400 transition-all ring-1 ring-emerald-500/20" title="Gerenciar Usuários (Admin)">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                    </a>
                    <a href="/admin/settings" className="w-8 h-8 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-xl flex items-center justify-center text-emerald-400 transition-all ring-1 ring-emerald-500/20" title="Configurações (Admin)">
                            ...
                            <tbody className="divide-y divide-white/5">
                              {filterAndSortReceipts(lot.receipts).map((rec: any, rIdx: number) => (
                                <tr key={rIdx} className="hover:bg-white/[0.04] transition-colors group/row">
                                  ...
                                  <td className="px-4 py-3 flex gap-1 justify-end relative">
                                    {(() => {
                                      const semContato = !rec.email && !rec.telefone;
                                      const carregando = resending === rec.id;
                                      return (
                                        <div className="relative">
                                          <button
                                            onClick={() => setResendMenuFor(resendMenuFor === rec.id ? null : rec.id)}
                                            disabled={semContato || carregando || dispatching}
                                            className="p-1 bg-[#1C1C1F] ring-1 ring-amber-500/30 hover:bg-amber-500/10 text-amber-400 hover:text-amber-300 rounded-lg transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                                            title={semContato ? 'Sem contato cadastrado' : 'Reenviar link existente (sem regenerar token)'}
                                          >
                                            {carregando ? (
                                          ...
                                          {resendMenuFor === rec.id && (
                                            <div className="absolute right-0 top-full mt-1 z-30 bg-[#0A0A0B] border border-white/10 rounded-xl shadow-2xl ring-1 ring-emerald-500/10 py-1 min-w-[160px]">
                                              <button
                                                onClick={() => resendReceipt(rec, 'email')}
                                                disabled={!rec.email}
                                                className="w-full text-left px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-zinc-300 hover:bg-white/5 disabled:text-zinc-700 disabled:hover:bg-transparent disabled:cursor-not-allowed flex items-center gap-2"
                                              >
                                                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                                                Por E-mail
                                              </button>
                                              <button
                                                onClick={() => resendReceipt(rec, 'whatsapp')}
                                                disabled={!rec.telefone}
                                                className="w-full text-left px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-emerald-400 hover:bg-white/5 disabled:text-zinc-700 disabled:hover:bg-transparent disabled:cursor-not-allowed flex items-center gap-2"
                                              >
                                                <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                                                Por WhatsApp
                                              </button>
                                              <button
                                                onClick={() => resendReceipt(rec, 'both')}
                                                disabled={!rec.email && !rec.telefone}
                                                className="w-full text-left px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-amber-300 hover:bg-white/5 disabled:text-zinc-700 disabled:hover:bg-transparent disabled:cursor-not-allowed border-t border-white/5"
                                              >
                                                Ambos os canais
                                              </button>
                                            </div>
